...
1. 一定要在路由之前注册中间件
2. 客户端发送过来的请求，可以联系调用多个中间件进行处理
3. 执行完中间件的业务代码后，一定要调用next函数
4. 为了防止逻辑混乱，不建议在next函数后再写额外的代码
5. 连续调用多个中间件时，多个中间件之间，共享req和res对象

### 4.4 中间件的分类
- **应用级别的中间件**
通过`app.use()`,`app.get()`,`app.post()`，绑定到**app实例**上的中间件，叫做应用级别的中间件。

- **路由级别的中间件**
绑定到**express.router()**实例上的中间件，叫做路由级别的中间件。它的用法和应用级别中间件没有区别。应用级别中间件时绑定在app实例上，路由级别中间件绑定在router实例上。

- **错误级别的中间件**
作用：专门用于捕获整个项目中发生的异常错误，从而防止项目异常崩溃的问题
格式：错误级别中间件的处理函数中，必须有4个形参，形参顺序从前到后，依次是`(err,req,res,next)`

- **Express内置的中间件**
    - `express.static`快速托管静态资源的内置中间件，例如HTML文件，图片，css样式等(无兼容性)
    ...
    - `express.urlencoded`解析URL-encoded格式的请求体数据(有兼容性，仅在4.16.0+版本中可用)
       使用方法:`app.use(express.urlencoded({extended:false}))` 
...
1. 使用`npm install cors`安装该模块
2. 使用`const CORS解决跨域 = require('cors')`导入中间件
3. 在路由之前调用`app.use(cors())`配置全局中间件
